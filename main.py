...
def delete_course(course):
    reponse = messagebox.askyesno("Student Information System","Delete this course?")
    if reponse == 0:
        return
...
def course_window(comm,course):
    ...
    def command():
        ...
        if code.get("1.0",END)=="\n" or cname.get("1.0",END)=="\n":
            messagebox.showinfo("Student Information System","Fill in all the fields.",parent=cWindow)
        else:
            if comm == "edit":
    ...
    for course in courseList:
        ...
        if searchword.lower() in course[2].lower():
            if column==3:
                row += 1
                column = 0
            currFrame=Frame(listFrame, highlightbackground="#971010", highlightthickness=3,bg="#ff9500",height=250, width=306)
            currFrame.grid(row=row,column=column,padx=(15,0),pady=(15,0))
            currFrame.grid_propagate(0)
            Label(currFrame,text=course[1],bg="#ff9500",font='Helvitica 30 bold',fg="white").grid(row=0,column=0,columnspan=2,padx=(20,0),pady=(30,0),sticky=W+E)
            Label(currFrame,text=course[2],bg="#ff9500",font='Helvitica 12 italic',fg="white").grid(row=1,column=0,columnspan=2,padx=(20,0),pady=(5,30),sticky=W+E)
            viewButton = Button(currFrame,image=button_viewstudents, borderwidth=0,bg='#ff9500',command=lambda x=course[0]:diverge(1,x))
            viewButton.image=button_viewstudents
            viewButton.grid(row=2,column=0,columnspan=2,padx=(20,0),pady=1)
            editButton = Button(currFrame,image=button_update1, borderwidth=0,bg='#ff9500',command=lambda x=course:course_window("edit",x))
            editButton.image=button_update1
            editButton.grid(row=3,column=0,padx=(20,0),pady=1)
            deleteButton = Button(currFrame,image=button_delete1, borderwidth=0,bg='#ff9500',command=lambda x=course:delete_course(x))
...
def delete_student(number):
    reponse = messagebox.askyesno("Student Information System","Delete this student?")
    if reponse == 0:
        return
...
def student_window(comm,student,strainer):
    ...
    def command():
        ...
        if ID.get()=="" or name.get()=="" or thiscourse.get()=="" or year.get()=="" or gender.get()=="":
            ...
            if len(ID.get().split("-")) != 2:
                messagebox.showinfo("Student Information System","Invalid ID format(yyyy-nnnn).",parent=sWindow)
                return
            ...
            if thiscourse.get() not in nameList:
                messagebox.showinfo("Student Information System","Course not found.",parent=sWindow)
                return
            #Checks if an existing record already exist
            for student in studentList:
                if student[0] == ID.get() or student[1] == name.get():
                    if comm == "edit" and student[0] == ID.get():
                        continue
                    messagebox.showinfo("Student Information System","Record already exists.",parent=sWindow)
                    return
    ...
    thisFrame = LabelFrame(sWindow,bg="#faf8f4")
    ...
    cancel = Button(tempFrame, image=button_cancel, borderwidth=0,bg='#faf8f4', command=sWindow.destroy)
    cancel.image=button_cancel
    cancel.grid(row=0,column=0, padx=5, pady=5)
    add = Button(tempFrame, image=button_add if command=="add" else button_save, borderwidth=0,bg='#faf8f4', command=command)
    ...
    for student in filtered:
        if (student[0].startswith(searchword) or (student[1].lower()).startswith(searchword.lower())):
            ...
            if column==2:
                row+=1
                column=0
            currFrame = Frame(listFrame, bg="white", highlightbackground="#484444", highlightthickness=3, height=100, width=470)
            ...
            column += 1
    fixFrame = Frame(listFrame, height=1000, width=470, bg="white")
...
Label(header, text="STUDENT INFORMATION SYSTEM", font = 'Helvitica 32 bold', bg="#971010", fg="white").place(relx=.4, rely=.40, anchor='c')
Label(header, text="a simple demonstration", font = 'Helvitica 14 italic', bg="#971010", fg="white").place(relx=.15, rely=.7, anchor='c')

midFrame = LabelFrame(root,bg="#484444",highlightcolor="black")
...
wrapper = LabelFrame(root,bg="white")
wrapper.grid(row=2, column=0,columnspan=2)
mycanvas = Canvas(wrapper, width=975,height=540,bg="white")
listFrame= Frame(mycanvas,bg="white")
...
courseButton = Button(root,image=button_courses, borderwidth=0,bg='white',command=lambda:diverge(0,""))
courseButton.image=button_courses
courseButton.grid(row=3,column=0,pady=6,padx=(5,0))
studentButton = Button(root,image=button_students, borderwidth=0,bg='white',command=lambda:diverge(1,""))
